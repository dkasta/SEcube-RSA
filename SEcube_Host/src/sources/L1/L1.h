...
public:
	...
	void L1Encrypt(size_t plaintext_size, std::shared_ptr<uint8_t[]> plaintext, SEcube_ciphertext& encrypted_data, uint16_t algorithm, uint16_t algorithm_mode, se3Key key, uint8_t on_the_fly) override ;
	...
	void L1Decrypt(SEcube_ciphertext& encrypted_data, size_t& plaintext_size, std::shared_ptr<uint8_t[]>& plaintext, uint8_t on_the_fly) override ;
	...
	/** @brief Get the asymmetric key public part associated with the specified ID.
	* @param [in, out] k The structure which must hold the ID of the key to
	* get and will hold the read key.
	* @detail Throws exception in case of errors. */
	void L1AsymmKeyGet(se3Key& k);
	...
	void L1AsymmCrypt(size_t text_in_size, std::shared_ptr<uint8_t[]> text_in,
			bool public_key, se3Key key, uint8_t on_the_fly,
			size_t &text_out_size, std::shared_ptr<uint8_t[]> text_out)
			override ;
	/** @brief Add asymmetric key to SEcube device.
	 * @param [in] k The key information.
	 * @param [in] op The operation to be performed.
	 * @detail Throws exception in case of errors.
	 * When \p op = L1Commands::KeyOpEdit::SE3_KEY_OP_ADD_RSA \p k must be
	 * filled with all asymmetric key attributes (id, dataSize, asymmKey).
	 * When \p op = L1Commands::KeyOpEdit::SE3_KEY_OP_ADD_GEN_RSA \p k can
	 * be filled with id and dataSize only. */
	void L1AsymmKeyEdit(se3Key& k, uint16_t op) override ;
	 ...
	 * key attributes (dataSize, asymmKey).
	 * When \p on_the_fly = false \p key can be filled with id only. */
	void L1Sign(const size_t input_size, const std::shared_ptr<uint8_t[]> input_data,
			const se3Key key, uint8_t on_the_fly,
			size_t &sign_size, std::shared_ptr<uint8_t[]> &sign) override;
	 ...
	 * key attributes (dataSize, asymmKey).
	 * When \p on_the_fly = false \p key can be filled with id only. */
	void L1Verify(const size_t input_size, const std::shared_ptr<uint8_t[]> input_data,
			const se3Key key, uint8_t on_the_fly,
			const size_t sign_size, const std::shared_ptr<uint8_t[]> sign,
	...
	void L1CertificateEdit(const L1Commands::CertOpEdit op,
			const SEcube_certificate_info info) override;
	/** @brief Check if the certificate with the specified ID is stored to
	 * the SEcube.
	 * @param [in] certId The ID of the certificate to search.
	 * @param [out] found Boolean that stores the result of the search.
	 * True if the certificate is found, false otherwise.
	 * @detail Throws exception in case of errors. There is no limitation in
	 * terms of IDs that can be passed (everything in range from 0 to 2^32-1
	 * is fine). */
	void L1CertificateFind(uint32_t certId, bool& found) override;
	/** @brief Get the certificate associated with the specified ID in form
	* of PEM string.
	* @param [in] cert_id The ID of the certificate to get.
	* @param [out] cert The string holding the certificate
	* @detail Throws exception in case of errors. */
	void L1CertificateGet(const uint32_t cert_id, std::string &cert) override;
	/** @brief List the certificates stored to the SEcube device.
	 * @param [out] certlist The list of certificates inside the SEcube (ID).
	 * @detail Throws exception in case of errors. */
	void L1CertificateList(std::vector<uint32_t>& certList) override;
	// Other API
