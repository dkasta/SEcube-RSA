...
void L1::L1AsymmCrypt(size_t text_in_size, std::shared_ptr<uint8_t[]> text_in,
		bool public_key, se3Key key, uint8_t on_the_fly,
		size_t &text_out_size, std::shared_ptr<uint8_t[]> text_out) {
	L1AsymmCryptException exc;
	if (text_in == nullptr) {
		cout << "L1AsymmCrypt: bad input data" << endl;
		throw exc;
	}

	// build request
	int offset = L1Request::Offset::DATA;
	uint8_t op;
	if (public_key) {
		op = L1Commands::RSA_Options::SE3_RSA_ENCRYPT;
	} else {
		op = L1Commands::RSA_Options::SE3_RSA_DECRYPT;
	}
	this->base.FillSessionBuffer((unsigned char *)&op, offset, sizeof(op));
	offset += sizeof(op);
	this->base.FillSessionBuffer((unsigned char *)&on_the_fly, offset, sizeof(on_the_fly));
	offset += sizeof(on_the_fly);
	if (on_the_fly) {
		this->base.FillSessionBuffer((unsigned char *)&(key.dataSize), offset, sizeof(key.dataSize));
		offset += sizeof(key.dataSize);
		this->base.FillSessionBuffer((unsigned char *)key.asymmKey.N, offset, key.dataSize);
		offset += key.dataSize;
		this->base.FillSessionBuffer((unsigned char *)key.asymmKey.E, offset, key.dataSize);
		offset += key.dataSize;
		if (!public_key) {
			this->base.FillSessionBuffer((unsigned char *)&(key.asymmKey.D), offset, key.dataSize);
			offset += key.dataSize;
		}
	} else {
	...
	this->base.FillSessionBuffer((unsigned char *)text_in.get(), offset, text_in_size);
	offset += text_in_size;
	const auto data_size = (offset - L1Request::Offset::DATA);
	...
	try {
		...
		cout << "L1AsymmCrypt: error during TXRX" << endl;
		throw exc;
	}

	// check and parse response
	if (resp_size == 0) {
		cout << "L1AsymmCrypt: error during the request execution" << endl;
		throw exc;
	}

	auto buff = this->base.GetSessionBuffer();
	buff += L1Response::Offset::DATA;
	text_out_size = resp_size;
	memcpy(text_out.get(), buff, text_out_size);
}

void L1::L1Encrypt(size_t plaintext_size, std::shared_ptr<uint8_t[]> plaintext, SEcube_ciphertext& encrypted_data, uint16_t algorithm, uint16_t algorithm_mode, se3Key key, uint8_t on_the_fly) {
	...
	if((algorithm != L1Algorithms::Algorithms::AES) && (algorithm != L1Algorithms::Algorithms::AES_HMACSHA256) &&
			(algorithm != L1Algorithms::Algorithms::RSA )){
		throw std::invalid_argument("Invalid algorithm.");
	}
	if((algorithm != L1Algorithms::Algorithms::RSA) &&
	   (algorithm_mode != CryptoInitialisation::Modes::ECB) &&
	...
	try {
		if (algorithm == L1Algorithms::Algorithms::RSA) {
			auto cipher_buff = std::shared_ptr<uint8_t[]>(new uint8_t[L1Key::RSASize::MAX_SIZE_BYTES]);
			L1AsymmCrypt(plaintext_size, plaintext, true, key,
					on_the_fly,
					encrypted_data.ciphertext_size,
					cipher_buff);
			encrypted_data.ciphertext = std::unique_ptr<uint8_t[]>(new uint8_t[encrypted_data.ciphertext_size]);
			memcpy(encrypted_data.ciphertext.get(), cipher_buff.get(), encrypted_data.ciphertext_size);
			return;
		}
		L1CryptoInit(algorithm, algorithm_mode | CryptoInitialisation::Direction::ENCRYPT, key.id, encSessId);
...
void L1::L1Decrypt(SEcube_ciphertext& encrypted_data, size_t& plaintext_size, std::shared_ptr<uint8_t[]>& plaintext, uint8_t on_the_fly) {
	L1DecryptException decryptExc;
	uint16_t algorithm = encrypted_data.algorithm;
	uint16_t algorithm_mode = encrypted_data.mode;
	uint32_t key_id = encrypted_data.key.id;
	if((algorithm == L1Algorithms::Algorithms::HMACSHA256) || (algorithm == L1Algorithms::Algorithms::SHA256)){
	...
	try {
		if (algorithm == L1Algorithms::Algorithms::RSA) {
			auto plain_buff = std::shared_ptr<uint8_t[]>(new uint8_t[L1Key::RSASize::MAX_SIZE_BYTES]);
			auto cipher_buff = std::shared_ptr<uint8_t[]>(new uint8_t[L1Key::RSASize::MAX_SIZE_BYTES]);
			memcpy(cipher_buff.get(), encrypted_data.ciphertext.get(), encrypted_data.ciphertext_size);
			L1AsymmCrypt(encrypted_data.ciphertext_size, cipher_buff,
					false, encrypted_data.key, on_the_fly,
					plaintext_size, plain_buff);
			plaintext = std::unique_ptr<uint8_t[]>(new uint8_t[plaintext_size]);
			memcpy(plaintext.get(), plain_buff.get(), plaintext_size);
			return;
		}
...
void L1::L1Sign(const size_t input_size, const std::shared_ptr<uint8_t[]> input_data,
		const se3Key key, uint8_t on_the_fly, size_t &sign_size, std::shared_ptr<uint8_t[]> &sign) {
	L1SignatureException exc;
	if (input_data == nullptr) {
		cout << "L1Sign: bad input data" << endl;
		throw exc;
	}
	...
	this->base.FillSessionBuffer((unsigned char *)input_data.get(), offset, input_size);
	offset += input_size;
	const auto data_size = (offset - L1Request::Offset::DATA);
	...
	try {
		...
		cout << "L1Sign: error during TXRX" << endl;
		throw exc;
	}

	// check and parse response
	if(resp_size == 0) {
		cout << "L1Sign: error during the request execution" << endl;
		throw exc;
	}
	...
	sign_size = resp_size;
	sign = std::shared_ptr<uint8_t[]>(new uint8_t[sign_size]);
	memcpy(sign.get(), buff, sign_size);
}

void L1::L1Verify(const size_t input_size, const std::shared_ptr<uint8_t[]> input_data,
		const se3Key key, uint8_t on_the_fly, const size_t sign_size,
		const std::shared_ptr<uint8_t[]> sign, bool &verified) {
	L1SignatureException exc;
	if ((input_data == nullptr) || (sign == nullptr)) {
		cout << "L1Verify: bad input data" << endl;
		throw exc;
	}
	...
	this->base.FillSessionBuffer((unsigned char *)input_data.get(), offset, input_size);
	offset += input_size;
	this->base.FillSessionBuffer((unsigned char *)sign.get(), offset, sign_size);
	offset += sign_size;
	const auto data_size = (offset - L1Request::Offset::DATA);
	...
	try {
		...
		cout << "L1RSAVerify: error during TXRX" << endl;
		throw exc;
	}
...
void L1::L1CertificateEdit(const L1Commands::CertOpEdit op, const SEcube_certificate_info info) {
	L1Exception exc;

	if ((op != L1Commands::CertOpEdit::SE3_CERT_OP_ADD) &&
			(op != L1Commands::CertOpEdit::SE3_CERT_OP_DELETE)) {
		cout << "L1CertificateEdit: unknown operation" << endl;
		throw exc;
	}
	...
	this->base.FillSessionBuffer((unsigned char *)&info.cert_id, offset, sizeof(info.cert_id));
	offset += sizeof(info.cert_id);
	if (op == L1Commands::CertOpEdit::SE3_CERT_OP_ADD) {
		...
		this->base.FillSessionBuffer((unsigned char *)info.serial_str.data(), offset, 20 + 1);
		offset += 20 + 1;
		...
		offset += 14 + 1;
		const uint16_t issuer_name_len = info.issuer_name.length();
		this->base.FillSessionBuffer((unsigned char *)&issuer_name_len, offset, sizeof(issuer_name_len));
		offset += sizeof(issuer_name_len);
		this->base.FillSessionBuffer((unsigned char *)info.issuer_name.c_str(), offset, issuer_name_len + 1);
		offset += issuer_name_len + 1;
		const uint16_t subject_name_len = info.subject_name.length();
		this->base.FillSessionBuffer((unsigned char *)&subject_name_len, offset, sizeof(subject_name_len));
		offset += sizeof(subject_name_len);
		this->base.FillSessionBuffer((unsigned char *)info.subject_name.c_str(), offset, subject_name_len + 1);
	...
	try {
		...
		cout << "L1CertificateEdit: error during TXRX" << endl;
		throw exc;
	}
	...
	if((resp_size != 2) || (strncmp((const char *)(buff), "OK", 2) != 0)){
		cout << "L1CertificateEdit: response not \"OK\"" << endl;
		throw exc;
	}
}
...
void L1::L1CertificateFind(uint32_t certId, bool& found) {
	...
	try {
		...
		cout << "L1CertificateFind: error during TXRX" << endl;
		throw exc;
	}
	...
	if(strncmp((const char *)(buff), "OK", 2) != 0){
		cout << "L1CertificateFind: response not \"OK\"" << endl;
		throw exc;
	}
	buff += strlen("OK");
...
void L1::L1CertificateGet(const uint32_t cert_id, std::string &cert) {
	...
	try {
		...
		cout << "L1CertificateGet: error during TXRX" << endl;
		throw exc;
	}

	// check and parse response
	if(resp_size == 0){
		cout << "L1CertificateGet: error during the request execution" << endl;
		throw exc;
	}
	...
	char *cert_buff = new char[resp_size + 1];
	memcpy(cert_buff, buff, resp_size);
	cert_buff[resp_size] = 0;
	cert = cert_buff;
	free(cert_buff);
}
...
void L1::L1CertificateList(std::vector<uint32_t>& certList) {
	...
	uint32_t id;
	const auto list_len = (resp_size / sizeof(id));

	for (auto i = 0; i < (int)list_len; i++) {
		memcpy(&id, buff, sizeof(id));
		buff += sizeof(id);
		certList.push_back(id);
	}
}
...
void L1::L1AsymmKeyGet(se3Key& k) {
	...
	try {
		...
		cout << "L1AsymmKeyGet: error during TXRX" << endl;
		throw exc;
	}
	
	// check and parse response
	if(resp_size == 0){
		cout << "L1AsymmKeyGet: error during request execution" << endl;
		throw exc;
	}

	auto buff = this->base.GetSessionBuffer();
	buff += L1Response::Offset::DATA;
	const auto key_size = (resp_size / 2);

	k.dataSize = key_size;
	memcpy(k.asymmKey.N, buff, key_size);
	buff += key_size;
	memcpy(k.asymmKey.E, buff, key_size);
	k.asymmKey.D = nullptr;
	k.asymmKey.type = 0;
}

void L1::L1AsymmKeyEdit(se3Key& k, uint16_t op) {
	L1KeyEditException exc;

	if((op != L1Commands::KeyOpEdit::SE3_KEY_OP_ADD_RSA) &&
			(op != L1Commands::KeyOpEdit::SE3_KEY_OP_ADD_GEN_RSA)){
		throw exc;
	}
	...
	} else if (op == L1Commands::KeyOpEdit::SE3_KEY_OP_ADD_GEN_RSA) {
		rsa_op = L1Commands::RSA_Options::SE3_RSA_KEYGEN;
	} else {
		cout << "L1AsymmKeyEdit: invalid operation\n";
		throw exc;
	}
	this->base.FillSessionBuffer((uint8_t *)&(rsa_op), offset, sizeof(rsa_op));
	offset += sizeof(rsa_op);
	this->base.FillSessionBuffer((uint8_t *)&(k.id), offset, sizeof(k.id));
	offset += sizeof(k.id);
	this->base.FillSessionBuffer((uint8_t *)&(k.dataSize), offset, sizeof(k.dataSize));
	offset += sizeof(k.dataSize);
	this->base.FillSessionBuffer((uint8_t *)&(k.asymmKey.type), offset, sizeof(k.asymmKey.type));
	offset += sizeof(k.asymmKey.type);
	if (op == L1Commands::KeyOpEdit::SE3_KEY_OP_ADD_RSA) {
		if ((k.asymmKey.N == nullptr) || (k.asymmKey.E == nullptr)) {
			cout << "L1AsymmKeyEdit: trying to add incomplete RSA key\n";
			throw exc;
		}
		this->base.FillSessionBuffer((uint8_t *)k.asymmKey.N, offset, k.dataSize);
		offset += k.dataSize;
		this->base.FillSessionBuffer((uint8_t *)k.asymmKey.E, offset, k.dataSize);
		offset += k.dataSize;
		if (k.asymmKey.D != nullptr) {
			this->base.FillSessionBuffer((uint8_t *)k.asymmKey.D, offset, k.dataSize);
			offset += k.dataSize;
		}
	}
	const auto data_size = (offset - L1Request::Offset::DATA);
	...
	try {
		...
		cout << "L1RSAKeyGen: error during TXRX" << endl;
		throw exc;
	}
	
	// check response
	if((resp_size != 2) || strncmp((const char*)(this->base.GetSessionBuffer() +
			L1Response::Offset::DATA), "OK", 2) != 0) {
		cout << "L1RSAKeyGen: error in response" << endl;
		throw exc;
	}	
}
...
void L1::L1KeyEdit(se3Key& k, uint16_t op) {
    ...
    switch(op){
    case L1Commands::KeyOpEdit::SE3_KEY_OP_ADD_RSA:
    case L1Commands::KeyOpEdit::SE3_KEY_OP_ADD_GEN_RSA:
    	L1::L1AsymmKeyEdit(k, op);
    	break;
    default:
