...
public:
	se3Key key; /**< The key that was used to perform the crypto operation. */
...
public:
	...
	virtual void L1AsymmKeyEdit(se3Key& k, uint16_t op) = 0;
	virtual void L1FindKey(uint32_t key_id, bool& found) = 0;
	virtual void L1AsymmCrypt(size_t text_in_size,
			std::shared_ptr<uint8_t[]> text_in,
			bool public_key, se3Key key, uint8_t on_the_fly,
			size_t &text_out_size,
			std::shared_ptr<uint8_t[]> text_out) = 0;
	...
	virtual void L1Encrypt(size_t plaintext_size, std::shared_ptr<uint8_t[]> plaintext, SEcube_ciphertext& encrypted_data, uint16_t algorithm, uint16_t algorithm_mode, se3Key key, uint8_t on_the_fly) = 0;
	virtual void L1Decrypt(SEcube_ciphertext& encrypted_data, size_t& plaintext_size, std::shared_ptr<uint8_t[]>& plaintext, uint8_t on_the_fly) = 0;
	virtual void L1Digest(size_t input_size, std::shared_ptr<uint8_t[]> input_data, SEcube_digest& digest) = 0;
	virtual void L1Sign(const size_t input_size,
			const std::shared_ptr<uint8_t[]> input_data,
			const se3Key key, uint8_t on_the_fly, size_t &sign_size,
			std::shared_ptr<uint8_t[]> &sign) = 0;
	virtual void L1Verify(const size_t input_size, const std::shared_ptr<uint8_t[]> input_data,
			const se3Key key, uint8_t on_the_fly,
			const size_t sign_size, const std::shared_ptr<uint8_t[]> sign,
			bool &verified) = 0;
	virtual void L1CertificateEdit(const L1Commands::CertOpEdit op, const SEcube_certificate_info info) = 0;
	virtual void L1CertificateFind(uint32_t certId, bool& found) = 0;
	virtual void L1CertificateGet(const uint32_t cert_id, std::string &cert) = 0;
	virtual void L1CertificateList(std::vector<uint32_t>& certList) = 0;
