...
uint16_t dispatcher_RSA(uint16_t req_size, const uint8_t* req, uint16_t* resp_size, uint8_t* resp){
    se3_rsa_operation operation; // the type of operation to be executed
    memcpy(&operation, req, sizeof(operation));
    const uint8_t *req_no_op = (req + sizeof(operation));
    const uint16_t req_size_no_op = (req_size - sizeof(operation));

    switch (operation) {
    case SE3_RSA_KEYADD:
    	return se3_rsa_keyadd(req_size_no_op, req_no_op, resp_size, resp);
    case SE3_RSA_KEYGEN:
    ...
    case SE3_RSA_SIGN:
    case SE3_RSA_VERIFY:
    	return se3_rsa_operate(req_size, req, resp_size, resp);
...
uint16_t key_find(uint16_t req_size, const uint8_t* req, uint16_t* resp_size, uint8_t* resp)
{
    ...
    uint32_t keyid = 0;
    se3_flash_it it = { .addr = NULL };
    memcpy((void*)&keyid, req, sizeof(keyid));
    // check if there is already a key with same ID
    se3_flash_it_init(&it);
    *resp_size = 1;
    *resp = se3_key_find(keyid, &it);
